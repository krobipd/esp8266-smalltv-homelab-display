...
int main() {
    ...
    assert(strcmp(etag, "\"v0.2.3-21623\"") == 0);
    ...
    assert(strcmp(etag, neu) != 0);

    // --- Abgleich mit dem, was der Browser zurueckschickt ---
    assert(etagPasst("\"v0.2.3-21623\"", etag));          // Normalfall
    assert(etagPasst("W/\"v0.2.3-21623\"", etag));        // als schwach markiert
    assert(etagPasst(" \"v0.2.3-21623\" ", etag));        // mit Leerzeichen
    assert(etagPasst("\"alt-1\", \"v0.2.3-21623\"", etag));   // mehrere Kennungen
    assert(etagPasst("\"alt-1\", W/\"v0.2.3-21623\"", etag));
    assert(etagPasst("*", etag));                          // jede Fassung genuegt

    // Nichts davon darf passen -- sonst bekaeme der Browser ein 304 fuer eine Datei,
    // die er gar nicht hat, und die Seite bliebe leer.
    assert(!etagPasst("\"v0.2.3-2162\"", etag));           // kuerzer, sonst gleich
    assert(!etagPasst("\"v0.2.3-216230\"", etag));         // laenger, sonst gleich
    assert(!etagPasst("\"v0.2.2-21623\"", etag));          // andere Version
    assert(!etagPasst("", etag));                          // Kopfzeile fehlt
    assert(!etagPasst(nullptr, etag));
    assert(!etagPasst("\"v0.2.3-21623\"", ""));            // keine eigene Kennung

    // --- Die Nicht-geaendert-Antwort ---
    char kopf[KOPF304_BUF_SIZE];
    baue304Kopf(kopf, sizeof(kopf), etag);
    assert(strncmp(kopf, "HTTP/1.1 304 Not Modified\r\n", 27) == 0);
    assert(strstr(kopf, "ETag: \"v0.2.3-21623\"\r\n") != nullptr);
    assert(strstr(kopf, "Cache-Control: no-cache\r\n") != nullptr);
    ...
    assert(strstr(kopf, "Content-Length") == nullptr);
    assert(strstr(kopf, "Content-Type") == nullptr);
    // Sauber abgeschlossen, sonst wartet der Browser auf mehr.
    assert(strlen(kopf) >= 4 && strcmp(kopf + strlen(kopf) - 4, "\r\n\r\n") == 0);
